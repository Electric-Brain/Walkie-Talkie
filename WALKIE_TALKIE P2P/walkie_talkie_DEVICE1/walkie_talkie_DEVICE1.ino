/**
 * ============================================================
 *  ESP32 WALKIE-TALKIE — DEVICE #1
 *  MY  MAC: A4:F0:0F:5C:08:98
 *  PEER MAC: 14:33:5C:03:DF:C8  (Device #2)
...
// ─────────────────────────────────────────────
#define MY_EXPECTED_MAC   "A4:F0:0F:5C:08:98"
static const uint8_t PEER_MAC[6] = {0x14, 0x33, 0x5C, 0x03, 0xDF, 0xC8};
...
// Mic baseline — measured at idle, used to centre the signal
// Your ADC reads ~200-400 at idle instead of 2048 (mic module offset)
// This is auto-calibrated on first TX press
static int16_t gMicBaseline = 2048;
static bool    gBaselineSet = false;

static float gHpPrev   = 0.0f;
static float gHpOutput = 0.0f;
static float gAgcGain  = 4.0f;   // start higher since mic signal is weak
...
//  Call once before first TX — averages 200 ADC samples
//  to find the true DC midpoint of this mic module.
// ─────────────────────────────────────────────
...
static void IRAM_ATTR txTimerISR(void *arg) {
  ...
  // Subtract measured baseline so signal is centred around 0
  float centred = (float)(raw - gMicBaseline);

  float   filt = highPass(centred);
  float   gain = applyAGC(filt);

  // Shift back to 0–4095 range for encoding
...
void setup() {
  ...
  delay(800);
  Serial.println("\n========================================");
  Serial.println("  WALKIE-TALKIE DEVICE #1");
  Serial.println("========================================");
  ...
  WiFi.mode(WIFI_STA);
  WiFi.disconnect();
  // Force channel 1 so both boards are on same channel
  ...
  peer.channel = 1;       // must match esp_wifi_set_channel above
  peer.encrypt = false;
  peer.ifidx   = WIFI_IF_STA;

  // Remove first in case of stale registration from previous flash
...
void loop() {
  ...
  if (pttPressed && !gPttWasPressed) {
    // Auto-calibrate mic baseline on first press
